...
declaration_list : declaration_list COMMA ID
		  {
 		  ...
 		  | declaration_list COMMA ID LTHIRD CONST_INT RTHIRD //array after some declaration
 		  {
			...
			if (table->exists_in_current_scope($3->get_name())) {
        outerror << "Error at line " << lines << ": Multiple declaration of " << $3->get_name() << endl;
        error_count++;
		...
		if (table->exists_in_current_scope($1->get_name())) {
        outerror << "Error at line " << lines << ": Multiple declaration of " << $1->get_name() << endl;
        error_count++;
    	}
